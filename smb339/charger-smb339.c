...
// Driver information on the platform level
static struct platform_device smb339_device = {
	.name	= "smb339",
	.id	= -1,
	.dev	= {
		// Used to pass the i2c client pointer in platform methods
		.platform_data = NULL
	},
};
...
static struct platform_driver smb339_platform_driver = {
	.probe = smb339_platform_probe,
	.remove = smb339_platform_remove,
	.driver = {
		.name = "smb339",
		.owner = THIS_MODULE,
	},
...
static int smb339_i2c_probe (struct i2c_client *client)
{
	...
	smb339_device.dev.platform_data = client;

	rc = platform_device_register(&smb339_device);
	rc = platform_driver_register(&smb339_platform_driver);
